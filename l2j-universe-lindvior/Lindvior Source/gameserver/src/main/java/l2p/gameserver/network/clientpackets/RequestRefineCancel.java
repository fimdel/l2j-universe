...
import l2p.gameserver.network.serverpackets.ExVariationCancelResult;
    ...
    public static int getRemovalPrice(ItemTemplate item) {
        switch (item.getItemGrade().cry) {
            ...
            case ItemTemplate.CRYSTAL_R:
                if (item.getItemGrade() == ItemGrade.R)
                    return 1530000;
                else if (item.getItemGrade() == ItemGrade.R95)
                    return 5400000;
                else if (item.getItemGrade() == ItemGrade.R99)
                    return 14160000;
