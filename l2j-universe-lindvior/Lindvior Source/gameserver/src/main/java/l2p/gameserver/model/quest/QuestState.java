...
public class QuestState {
    ...
    public QuestState exitCurrentQuest(boolean repeatable) {
        ...
        if (isCompleted()) {
            // WorldStatisticsManager.getInstance().updateStat(player, CategoryType.QUESTS_COMPLETED,0, 1);
        }
    ...
    /**
     * Этот метод рассчитывает количество дропнутых вещей в зависимости от рейтов и дает их, проверяет максимум, а так же проигрывает звук получения вещи. <br>
     * <br>
     * Ни один из передаваемых параметров не должен быть равен 0
     *
     ...
     * @return QuestState
     */
    public QuestState exitQuest(boolean repeatable) { // Повторный метод, но он мне нужен
        Player player = getPlayer();
        if (player == null) {
            return this;
        }
