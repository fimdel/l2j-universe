...
/**
 * Коллекция предопределенных классов
 * функций.
 */
public class StatFunctions {
    private static class FuncMultRegenResting extends Func {
        ...
        public void calc(Env env) {
            if (env.character.isPlayer() && env.character.getLevel() <= 40
                    && ((Player) env.character).getClassLevel() < 2
                    && stat == Stats.REGENERATE_HP_RATE)
    ...
    private static class FuncPAtkMul extends Func {
        ...
        private FuncPAtkMul() {
            super(Stats.PHYSICAL_ATTACK, 0x20, null);
        }
    ...
    private static class FuncPDefMul extends Func {
        ...
        private FuncPDefMul() {
            super(Stats.PHYSICAL_DEFENCE, 0x20, null);
        }
    ...
    private static class FuncAttackRange extends Func {
        ...
        private FuncAttackRange() {
            super(Stats.PHYSICAL_ATTACK_RANGE, 0x20, null);
        }
    ...
    private static class FuncAccuracyAdd extends Func {
        ...
        private FuncAccuracyAdd() {
            super(Stats.PHYSICAL_ACCURACY_COMBAT, 0x10, null);
        }
    ...
    private static class FuncEvasionAdd extends Func {
        ...
        private FuncEvasionAdd() {
            super(Stats.PHYSICAL_EVASION_RATE, 0x10, null);
        }
    ...
    private static class FuncPCriticalRateMul extends Func {
        ...
        private FuncPCriticalRateMul() {
            super(Stats.PHYSICAL_CRIT_BASE, 0x10, null);
        }
        ...
        public void calc(Env env) {
            ...
            env.value *= 0.01 * env.character.calcStat(Stats.PHYSICAL_CRIT_RATE,
                    env.target, env.skill);
        }
    }
    ...
    private static class FuncPAtkSpeedMul extends Func {
        ...
        private FuncPAtkSpeedMul() {
            super(Stats.PHYSICAL_ATTACK_SPEED, 0x20, null);
        }
    ...
    private static class FuncPDamageResists extends Func {
        ...
        public void calc(Env env) {
            if (env.target.isRaid()
                    && env.character.getLevel() - env.target.getLevel() > Config.RAID_MAX_LEVEL_DIFF) {
                env.value = 1;
                return;
            }
            ...
            if (weapon == null)
                env.value *= 0.01 * env.target.calcStat(
                        Stats.FIST_WPN_VULNERABILITY, env.character, env.skill);
            ...
            env.value = Formulas.calcDamageResists(env.skill, env.character,
    ...
    private static class FuncMDamageResists extends Func {
        ...
        public void calc(Env env) {
            if (env.target.isRaid()
                    && Math.abs(env.character.getLevel()
                    - env.target.getLevel()) > Config.RAID_MAX_LEVEL_DIFF) {
                env.value = 1;
                return;
            }
            env.value = Formulas.calcDamageResists(env.skill, env.character,
    ...
    private static class FuncInventory extends Func {
        ...
        public void calc(Env env) {
            ...
            env.value = Math.min(env.value,
                    Config.SERVICES_EXPAND_INVENTORY_MAX);
        }
    }
    ...
    private static class FuncSDefPlayers extends Func {
        ...
        public void calc(Env env) {
            ...
            Creature cha = env.character;
            ItemInstance shld = ((Player) cha).getInventory().getPaperdollItem(
    ...
    private static class FuncPDefLimit extends Func {
        ...
        private FuncPDefLimit() {
            super(Stats.PHYSICAL_DEFENCE, 0x100, null);
        }
    ...
    private static class FuncPAtkLimit extends Func {
        ...
        private FuncPAtkLimit() {
            super(Stats.PHYSICAL_ATTACK, 0x100, null);
        }
    ...
    private static class FuncPAtkSpdLimit extends Func {
        ...
        private FuncPAtkSpdLimit() {
            super(Stats.PHYSICAL_ATTACK_SPEED, 0x100, null);
        }
    ...
    private static class FuncCAtkLimit extends Func {
        ...
        private FuncCAtkLimit() {
            super(Stats.PHYSICAL_CRIT_DAMAGE, 0x100, null);
        }
    ...
    private static class FuncEvasionLimit extends Func {
        ...
        private FuncEvasionLimit() {
            super(Stats.PHYSICAL_EVASION_RATE, 0x100, null);
        }
    ...
    private static class FuncAccuracyLimit extends Func {
        ...
        private FuncAccuracyLimit() {
            super(Stats.PHYSICAL_ACCURACY_COMBAT, 0x100, null);
        }
    ...
    private static class FuncCritLimit extends Func {
        ...
        private FuncCritLimit() {
            super(Stats.PHYSICAL_CRIT_BASE, 0x100, null);
        }
    ...
    private static class FuncMCritLimit extends Func {
        ...
        private FuncMCritLimit() {
            super(Stats.MAGIC_CRIT_RATE, 0x100, null);
        }
    ...
    private static class FuncAttributeAttackInit extends Func {
        ...
        @Override
        public void calc(Env env) {
            env.value += env.character.getTemplate().getBaseAttributeAttack()[element
                    .getId()];
        }
    }
    ...
    private static class FuncAttributeDefenceInit extends Func {
        ...
        @Override
        public void calc(Env env) {
            env.value += env.character.getTemplate().getBaseAttributeDefence()[element
                    .getId()];
        }
    }
    ...
    private static class FuncAttributeAttackSet extends Func {
        ...
        static {
            for (int i = 0; i < Element.VALUES.length; i++)
                func[i] = new FuncAttributeAttackSet(
                        Element.VALUES[i].getAttack());
        }
    ...
    private static class FuncAttributeDefenceSet extends Func {
        ...
        static {
            for (int i = 0; i < Element.VALUES.length; i++)
                func[i] = new FuncAttributeDefenceSet(
                        Element.VALUES[i].getDefence());
        }
    ...
    private static class FuncHpRegenMul extends Func {
        ...
        public void calc(Env env) {
            env.value *= BaseStats.CON.calcBonus(env.character);
            if (env.character.isServitor()) //TODO: [Darvin] Наверно пет тоже?
                env.value *= 2;
        }
    }
    ...
    public static void addPredefinedFuncs(Creature cha) {
        if (cha.isPlayer()) {
            ...
            cha.addStatFunc(FuncAttributeAttackInit.getFunc(Element.FIRE));//TODO
            cha.addStatFunc(FuncAttributeAttackInit.getFunc(Element.WATER));//TODO
            cha.addStatFunc(FuncAttributeAttackInit.getFunc(Element.EARTH));//TODO
            cha.addStatFunc(FuncAttributeAttackInit.getFunc(Element.WIND));//TODO
            cha.addStatFunc(FuncAttributeAttackInit.getFunc(Element.HOLY));//TODO
            cha.addStatFunc(FuncAttributeAttackInit.getFunc(Element.UNHOLY));//TODO

            cha.addStatFunc(FuncAttributeDefenceInit.getFunc(Element.FIRE));//TODO
            cha.addStatFunc(FuncAttributeDefenceInit.getFunc(Element.WATER));//TODO
            cha.addStatFunc(FuncAttributeDefenceInit.getFunc(Element.EARTH));//TODO
            cha.addStatFunc(FuncAttributeDefenceInit.getFunc(Element.WIND));//TODO
            cha.addStatFunc(FuncAttributeDefenceInit.getFunc(Element.HOLY));//TODO
            cha.addStatFunc(FuncAttributeDefenceInit.getFunc(Element.UNHOLY));//TODO
        ...
        if (cha.isPlayable()) //TODO: [Darvin] Проверить на оффе.
        {
    ...
    private static class FuncMAccuracyAdd extends Func {
        ...
        private FuncMAccuracyAdd() {
            super(Stats.MAGIC_ACCURACY, 0x10, null);
        }
    ...
    private static class FuncMEvasionAdd extends Func {
        ...
        private FuncMEvasionAdd() {
            super(Stats.MAGIC_EVASION_RATE, 0x10, null);
        }
    ...
    private static class FuncMCriticalRateMul extends Func {
        ...
        private FuncMCriticalRateMul() {
            super(Stats.MAGIC_CRIT_RATE, 0x10, null);
        }
