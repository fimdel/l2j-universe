...
public abstract class Creature extends GameObject {
    ...
    private boolean _isDoubleCastingNow = false;
    private AtomicState _deathImmunity = new AtomicState();
    ...
    public final void stopKnockDown() {
        AbnormalStatusUpdate mi = new AbnormalStatusUpdate();
     ...
     */
    private TIntHashSet _abnormalEffects;
    ...
    private List<Zone> _zones = new ArrayList<Zone>();
    /**
    ...
    public void callSkill(Skill skill, List<Creature> targets, boolean useActionSkills) {
        try {
            ...
            while (itr.hasNext()) {
                ...
                if (skill.getAlterSkillId() > -1 && skill.getAlterSkillTime() > -1) {
                    if (pl.getTargetId() == pl.getTargetId())
                        pl.sendPacket(new ExAlterSkillRequest(skill.getAlterSkillId(), skill.getId(), skill.getAlterSkillTime())); // TODO: Check and rework me!
                }
    ...
    private void doAttackHitByPole(Attack attack, Creature target, int sAtk) {
        ...
        int range = (int) calcStat(Stats.PHYSICAL_ATTACK_RANGE, getTemplate().getBaseAtkRange(), target, null);
    ...
    protected void onDeath(Creature killer) {
        if (killer != null) {
            ...
            if (killerPlayer != null) {
                ...
                //WorldStatisticsManager.getInstance().updateStat(killerPlayer, CategoryType.MONSTERS_KILLED, 0, 1L);
            }
    ...
    public final int[] getAbnormalEffects() {
        return (_abnormalEffects == null) ? ArrayUtils.EMPTY_INT_ARRAY : _abnormalEffects.toArray();
    }

    public int getAccuracy() {
        return (int) calcStat(Stats.PHYSICAL_ACCURACY_COMBAT, 0.0, null, null);
    }

    public int getMAccuracy() {
        return (int) calcStat(Stats.MAGIC_ACCURACY, 0.0, null, null);
    }

    public int getEvasionRate(Creature target) {
        return (int) calcStat(Stats.PHYSICAL_EVASION_RATE, 0.0, target, null);
    }

    public int getMEvasionRate(Creature target) {
        return (int) calcStat(Stats.MAGIC_EVASION_RATE, 0.0, target, null);
    }
    ...
    public int getCriticalHit(Creature target, Skill skill) {
        return (int) calcStat(Stats.PHYSICAL_CRIT_BASE, _template.getBaseCritRate(), target, skill);
    }

    /**
     * Возвращает шанс магического крита в процентах
     */
    public int getMagicCriticalRate(Creature target, Skill skill) {
        return (int) calcStat(Stats.MAGIC_CRIT_RATE, _template.getBaseMageCritRate(), target, skill);
    }
    ...
    public int getPAtk(Creature target) {
        return (int) calcStat(Stats.PHYSICAL_ATTACK, _template.getBasePAtk(), target, null);
    }

    public int getPAtkSpd() {
        return (int) calcStat(Stats.PHYSICAL_ATTACK_SPEED, _template.getBasePAtkSpd(), null, null);
    }

    public int getPDef(Creature target) {
        return (int) calcStat(Stats.PHYSICAL_DEFENCE, _template.getBasePDef(), target, null);
    }

    public final int getPhysicalAttackRange() {
        return (int) calcStat(Stats.PHYSICAL_ATTACK_RANGE, getTemplate().getBaseAtkRange(), null, null);
    }
    ...
    public void onCastEndTime(boolean success) {
        ...
        if (_castingSkill != null) {
            skillId = this._castingSkill.getId();
            if (success && (
                    skillId == 10040 || skillId == 11776 || skillId == 11276 || skillId == 11052)) {
                useTriggers(getTarget(), TriggerType.ON_SUCCES_FINISH_CAST, null, _castingSkill, 0.0D);
            }
        }
        clearCastVars();
    ...
    public void reduceCurrentHp(double damage, double reflectableDamage, Creature attacker, Skill skill, boolean awake, boolean standUp, boolean directHp, boolean canReflect, boolean transferDamage, boolean isDot, boolean sendMessage) {
        ...
        /*  if (attacker.isPlayer()) {
         //WorldStatisticsManager.getInstance().updateStat(attacker.getPlayer(), CategoryType.DAMAGE_TO_MONSTERS, attacker.getPlayer().getActiveClassId(), (long) damage);
          if(attacker.getPlayer().getActiveClassId() == 139)
             //WorldStatisticsManager.getInstance().updateStat(attacker.getPlayer(), CategoryType.DAMAGE_TO_MONSTERS_SIGEL_KNIGHT, attacker.getPlayer().getActiveClassId(), (long) damage);
          else if (attacker.getPlayer().getActiveClassId() == 140)
              //WorldStatisticsManager.getInstance().updateStat(attacker.getPlayer(), CategoryType.DAMAGE_TO_MONSTERS_TYRR_WARRIOR, attacker.getPlayer().getActiveClassId(), (long) damage);
          else if (attacker.getPlayer().getActiveClassId() == 141)
              //WorldStatisticsManager.getInstance().updateStat(attacker.getPlayer(), CategoryType.DAMAGE_TO_MONSTERS_YUL_ARCHER, attacker.getPlayer().getActiveClassId(), (long) damage);
          else if (attacker.getPlayer().getActiveClassId() == 142)
              //WorldStatisticsManager.getInstance().updateStat(attacker.getPlayer(), CategoryType.DAMAGE_TO_MONSTERS_OTHELL_ROGUE, attacker.getPlayer().getActiveClassId(), (long) damage);
          else if (attacker.getPlayer().getActiveClassId() == 143)
              //WorldStatisticsManager.getInstance().updateStat(attacker.getPlayer(), CategoryType.DAMAGE_TO_MONSTERS_ISS_ENCHANTER, attacker.getPlayer().getActiveClassId(), (long) damage);
          else if (attacker.getPlayer().getActiveClassId() == 144)
              //WorldStatisticsManager.getInstance().updateStat(attacker.getPlayer(), CategoryType.DAMAGE_TO_MONSTERS_FEOH_WIZARD, attacker.getPlayer().getActiveClassId(), (long) damage);
          else if (attacker.getPlayer().getActiveClassId() == 145)
              //WorldStatisticsManager.getInstance().updateStat(attacker.getPlayer(), CategoryType.DAMAGE_TO_MONSTERS_WYNN_SUMMONER, attacker.getPlayer().getActiveClassId(), (long) damage);
          else if (attacker.getPlayer().getActiveClassId() == 146)
              //WorldStatisticsManager.getInstance().updateStat(attacker.getPlayer(), CategoryType.DAMAGE_TO_MONSTERS_AEORE_HEALER, attacker.getPlayer().getActiveClassId(), (long) damage);

         //WorldStatisticsManager.getInstance().updateStat(attacker.getPlayer(), CategoryType.DAMAGE_TO_MONSTERS_MAX, attacker.getPlayer().getActiveClassId(), (long) damage);
         if(attacker.getPlayer().getActiveClassId() == 139)
             //WorldStatisticsManager.getInstance().updateStat(attacker.getPlayer(), CategoryType.DAMAGE_TO_MONSTERS_MAX_SIGEL_KNIGHT, attacker.getPlayer().getActiveClassId(), (long) damage);
         else if (attacker.getPlayer().getActiveClassId() == 140)
             //WorldStatisticsManager.getInstance().updateStat(attacker.getPlayer(), CategoryType.DAMAGE_TO_MONSTERS_MAX_TYRR_WARRIOR, attacker.getPlayer().getActiveClassId(), (long) damage);
         else if (attacker.getPlayer().getActiveClassId() == 141)
             //WorldStatisticsManager.getInstance().updateStat(attacker.getPlayer(), CategoryType.DAMAGE_TO_MONSTERS_MAX_YUL_ARCHER, attacker.getPlayer().getActiveClassId(), (long) damage);
         else if (attacker.getPlayer().getActiveClassId() == 142)
             //WorldStatisticsManager.getInstance().updateStat(attacker.getPlayer(), CategoryType.DAMAGE_TO_MONSTERS_MAX_OTHELL_ROGUE, attacker.getPlayer().getActiveClassId(), (long) damage);
         else if (attacker.getPlayer().getActiveClassId() == 143)
             //WorldStatisticsManager.getInstance().updateStat(attacker.getPlayer(), CategoryType.DAMAGE_TO_MONSTERS_MAX_ISS_ENCHANTER, attacker.getPlayer().getActiveClassId(), (long) damage);
         else if (attacker.getPlayer().getActiveClassId() == 144)
             //WorldStatisticsManager.getInstance().updateStat(attacker.getPlayer(), CategoryType.DAMAGE_TO_MONSTERS_MAX_FEOH_WIZARD, attacker.getPlayer().getActiveClassId(), (long) damage);
         else if (attacker.getPlayer().getActiveClassId() == 145)
             //WorldStatisticsManager.getInstance().updateStat(attacker.getPlayer(), CategoryType.DAMAGE_TO_MONSTERS_MAX_WYNN_SUMMONER, attacker.getPlayer().getActiveClassId(), (long) damage);
         else if (attacker.getPlayer().getActiveClassId() == 146)
             //WorldStatisticsManager.getInstance().updateStat(attacker.getPlayer(), CategoryType.DAMAGE_TO_MONSTERS_MAX_AEORE_HEALER, attacker.getPlayer().getActiveClassId(), (long) damage);
     }   */
    }
    ...
    public void reduceCurrentMp(double i, Creature attacker) {
        ...
        if (attacker != null && attacker.isPlayer() && Math.abs(attacker.getLevel() - getLevel()) > 10) {
            ...
            if (attacker.isPK() && getEffectList().getEffectsBySkillId(5182) != null && !isInZone(ZoneType.SIEGE))
                return;
            // чар с блессингом не может нанести урон ПК
            if (isPK() && attacker.getEffectList().getEffectsBySkillId(5182) != null && !attacker.isInZone(ZoneType.SIEGE))
                return;
        }
    ...
    public void startAbnormalEffects(final int[] abnormalEffects) {
        if (_abnormalEffects == null) {
            _abnormalEffects = new TIntHashSet();
        }

        _abnormalEffects.addAll(abnormalEffects);
        sendChanges();
    }
    ...
    public void startAbnormalEffect(AbnormalEffect ae) {
        if (ae == AbnormalEffect.NULL) {
            ...
            _abnormalEffects.add(ae.ordinal());
        }
    ...
    public void stopAbnormalEffects(final int[] abnormalEffects) {
        if (_abnormalEffects != null) {
            _abnormalEffects.removeAll(abnormalEffects);
            sendChanges();
        }
    }

    public void stopAbnormalEffect(AbnormalEffect ae) {
        if ((_abnormalEffects != null) && _abnormalEffects.contains(ae.ordinal())) {
            _abnormalEffects.remove(ae.ordinal());
            sendChanges();
        }
    }
    ...
    public void setDoubleCastingNow(boolean cast) {
        _isDoubleCastingNow = cast;
    }

    public boolean allowDualCastSkill(final Skill skill) {
        if (skill.allowDual() && (getEffectList().getEffectByType(EffectType.DoubleCasting) != null)) {
            if (isCastingNow()) {
                if (!skill.allowDual()) {
                    return true;
                }
    ...
    public Skill removeSkill(Integer id) {
        Skill oldSkill = _skills.remove(id);
        ...
        if (oldSkill != null) {
            ...
            if (oldSkill.hasPassiveEffects()) {
                this._effectList.stopEffect(oldSkill.getId());
            }
        }
        return oldSkill;
    }
