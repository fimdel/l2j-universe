...
import l2p.gameserver.network.serverpackets.company.pledge.*;
...
public class Clan implements Iterable<UnitMember> {
    ...
    public void updateClanInDB() {
        ...
        try {
            ...
            statement = con.prepareStatement("UPDATE clan_data SET ally_id=?,reputation_score=?,expelled_member=?,leaved_ally=?,dissolved_ally=?,clan_level=?,warehouse=?,airship=?,academy_graduates=? WHERE clan_id=?");
    ...
    public static Clan restore(int clanId) {
        ...
        try {
            ...
            statement1 = con1.prepareStatement("SELECT clan_level,hasCastle,hasFortress,hasHideout,ally_id,reputation_score,expelled_member,leaved_ally,dissolved_ally,warehouse,airship,academy_graduates FROM clan_data where clan_id=?");
