...
import l2p.gameserver.templates.npc.RandomActions;
import l2p.gameserver.templates.npc.WalkerRoute;
import l2p.gameserver.templates.npc.WalkerRoutePoint;
...
public class NpcAI extends CharacterAI {
    ...
    private void continueWalkerRoute() {
        if (_haveWalkerRoute) {
            ...
            if (socialActionId >= 0) {
                actor.broadcastPacket(new SocialAction(actor.getObjectId(), socialActionId));
            }
