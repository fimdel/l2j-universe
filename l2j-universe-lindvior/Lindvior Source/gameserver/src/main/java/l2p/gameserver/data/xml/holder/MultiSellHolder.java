...
import l2p.gameserver.network.serverpackets.MultiSellList;
        ...
        if (entry.getIngredients().size() == 1 && entry.getProduction().size() == 1 && entry.getIngredients().get(0).getItemId() == 57) {
            ...
            if (item.getReferencePrice() > entry.getIngredients().get(0).getItemCount() && !Config.LOGIN_SERVER_IS_PVP)
                _log.warn("MultiSell [" + multiSellId + "] Production '" + item.getName() + "' [" + entry.getProduction().get(0).getItemId()
                        + "] price is lower than referenced | " + item.getReferencePrice() + " > " + entry.getIngredients().get(0).getItemCount());
        }
            ...
            if (showall)
                ...
                for (MultiSellIngredient ingredient : ingridients) {
                    ...
                    if (ingredient.getItemId() <= 0 || nokey || template.isEquipment()) {
                        if (ingredient.getItemId() == 12374) // Mammon's Varnish
                            // Enhancer
                            continue;
                        ...
                        for (final ItemInstance item : items)
                            if (item.getItemId() == ingredient.getItemId() && item.canBeExchanged(player)) {
                                ...
                                if (item.getEnchantLevel() < ingredient.getItemEnchant()) // Некоторые
                                    // мультиселлы
                                    // требуют
                                    // заточки
                                    continue;
                                ...
                                MultiSellEntry possibleEntry = new MultiSellEntry(enchant ? ent.getEntryId() + item.getEnchantLevel() * 100000
                                        : ent.getEntryId());
