...
import l2p.gameserver.templates.player.*;
...
public final class PlayerTemplateParser extends AbstractDirParser<PlayerTemplateHolder> {
    ...
    public File getXMLDir() {
        return new File(Config.DATAPACK_ROOT, "data/stats/pc_parameters/template_data/");
    }
                        ...
                        if ("start_equipments".equalsIgnoreCase(e.getName())) {
                            for (Element e2 : e.elements()) {
                                if ("equipment".equalsIgnoreCase(e2.getName())) {
                                    ...
                                    boolean warehouse = Boolean.parseBoolean(e2.attributeValue("warehouse", "false"));
                                    start_items.add(new StartItem(item_id, count, equiped, warehouse));
                                }
                            }
