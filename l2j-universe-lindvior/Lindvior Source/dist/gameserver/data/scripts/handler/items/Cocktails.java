...
public class Cocktails extends SimpleItemHandler {
    ...
    //Elixir of Blessing
    private static final int[] ELIXIR_OF_BLESSING_LIST = {9198, 9200, 9201, 9202, 9203};
        ...
        switch (itemId) {
            ...
            case 33862:
                for (int skill_id : ELIXIR_OF_BLESSING_LIST) {
                    player.broadcastPacket(new MagicSkillUse(player, player, skill_id, 1, 0, 0));
                    player.altOnMagicUseTimer(player, SkillTable.getInstance().getInfo(skill_id, 1));
                }
                break;
            default:
                return false;
