 ...
 */
public class TvTArena8 extends Functions implements ScriptFile, OnDeathListener, OnTeleportListener, OnPlayerExitListener {
    private static final Logger _log = LoggerFactory.getLogger(TvTArena8.class);

    private static List<SimpleSpawner> _spawns = new ArrayList<SimpleSpawner>();
    private static int EVENT_MANAGER_ID = 36619;
    ...
    public void onLoad() {
        ...
        if (Config.EVENT_TVT_ARENA_ENABLED && !isActive()) {
            scheduleEventStart();
            scheduleEventStop();
            _log.info("Loaded Event: TVT Arena 8 [state: activated]");
        } else if (Config.EVENT_TVT_ARENA_ENABLED && isActive()) {
            spawnEventManagers();
            _log.info("Loaded Event: TVT Arena 8 [state: started]");
        } else
            _log.info("Loaded Event: TVT Arena 8 [state: deactivated]");
    }

    public boolean isActive() {
        return IsActive("TVTArena8");
    }
    ...
    public void scheduleEventStart() {
        try {
            ...
            _log.warn("TVT Arena 8: Error figuring out a start time. Check EventStartTime in config file.");
        }
    }
    ...
    public void scheduleEventStop() {
        try {
            ...
            _log.warn("TVT Arena 8: Error figuring out a stop time. Check EventStopTime in config file.");
        }
    }
    ...
    public class StartTask extends RunnableImpl {
        ...
        public void runImpl() {
            spawnEventManagers();
            Announcements.getInstance().announceToAll("Начался евент TVT Arena 8");
            _log.info("Event TVT Arena 8 started");
        }
    }
    ...
    public class StopTask extends RunnableImpl {
        ...
        public void runImpl() {
            ...
            Announcements.getInstance().announceToAll("Окончен евент TVT Arena 8");
            _log.info("Event TVT Arena 8 stopped");
        }
    }
    ...
    public String DialogAppend_36616(Integer val) {
        ...
        if (val == 0)
            return HtmCache.getInstance().getNotNull("scripts/events/TVTArena/36619.htm", player);
        return "";
    }
    ...
    public void startEvent() {
        ...
        if (SetActive("TVTArena5", true)) {
            ...
            player.sendMessage("Event: TVT Arena 8 already started");
    ...
    public void stopEvent() {
        ...
        if (SetActive("TVTArena8", false)) {
            ServerVariables.unset("TVTArena8");
            unSpawnEventManagers();
            stop();
            _log.info("Event: TVT Arena 8 stopped");
            Announcements.getInstance().announceToAll("Окончен евент TVT Arena 8");
        } else
            player.sendMessage("Event: TVT Arena 8 not started");
