 ...
 */
public class TvTArena7 extends Functions implements ScriptFile, OnDeathListener, OnTeleportListener, OnPlayerExitListener {
    private static final Logger _log = LoggerFactory.getLogger(TvTArena7.class);

    private static List<SimpleSpawner> _spawns = new ArrayList<SimpleSpawner>();
    private static int EVENT_MANAGER_ID = 36618;
    ...
    public void onLoad() {
        ...
        if (Config.EVENT_TVT_ARENA_ENABLED && !isActive()) {
            scheduleEventStart();
            scheduleEventStop();
            _log.info("Loaded Event: TVT Arena 7 [state: activated]");
        } else if (Config.EVENT_TVT_ARENA_ENABLED && isActive()) {
            spawnEventManagers();
            _log.info("Loaded Event: TVT Arena 7 [state: started]");
        } else
            _log.info("Loaded Event: TVT Arena 7 [state: deactivated]");
    }

    public boolean isActive() {
        return IsActive("TVTArena7");
    }
    ...
    public void scheduleEventStart() {
        try {
            ...
            _log.warn("TVT Arena 7: Error figuring out a start time. Check EventStartTime in config file.");
        }
    }
    ...
    public void scheduleEventStop() {
        try {
            ...
            _log.warn("TVT Arena 7: Error figuring out a stop time. Check EventStopTime in config file.");
        }
    }
    ...
    public class StartTask extends RunnableImpl {
        ...
        public void runImpl() {
            spawnEventManagers();
            Announcements.getInstance().announceToAll("Начался евент TVT Arena 7");
            _log.info("Event TVT Arena 7 started");
        }
    }
    ...
    public class StopTask extends RunnableImpl {
        ...
        public void runImpl() {
            ...
            Announcements.getInstance().announceToAll("Окончен евент TVT Arena 7");
            _log.info("Event TVT Arena 7 stopped");
        }
    }
    ...
    public String DialogAppend_36616(Integer val) {
        ...
        if (val == 0)
            return HtmCache.getInstance().getNotNull("scripts/events/TVTArena/36618.htm", player);
        return "";
    }
    ...
    public void startEvent() {
        ...
        if (SetActive("TVTArena7", true)) {
            spawnEventManagers();
            _log.info("Event: TVT Arena 7 started");
            Announcements.getInstance().announceToAll("Начался евент TVT Arena 7");
        } else
            player.sendMessage("Event: TVT Arena 7 already started");
    ...
    public void stopEvent() {
        ...
        if (SetActive("TVTArena7", false)) {
            ServerVariables.unset("TVTArena7");
            unSpawnEventManagers();
            stop();
            _log.info("Event: TVT Arena 7 stopped");
            Announcements.getInstance().announceToAll("Окончен евент TVT Arena 7");
        } else
            player.sendMessage("Event: TVT Arena 7 not started");
