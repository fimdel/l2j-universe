...
public class SpezionNormal extends Reflection {
    ...
    private class Fail extends RunnableImpl {
        ...
        public void runImpl() throws Exception {
            ...
            for (Player player : getPlayers()) {
                player.sendPacket(new ExShowScreenMessage(NpcString.LOCATION_PORTAL_CHANGED, 5000, ExShowScreenMessage.ScreenMessageAlign.TOP_CENTER, true, 1, -1, true));
    ...
    public void openGate(int id) {
        if (!getDoor(id).isOpen()) {
            ...
            if (id == 26190004)
                for (Player player : getPlayers())
                    player.sendPacket(new ExShowScreenMessage(NpcString.THE_DOOR_OPENED, 5000, ExShowScreenMessage.ScreenMessageAlign.TOP_CENTER, true, 1, -1, true));
            else
                for (Player player : getPlayers())
                    player.sendPacket(new ExShowScreenMessage(NpcString.THE_DOOR_OPENED_SOMEONE_HAS_TO_STAY_AND_WATCH_FOR_A_TIME_BOMB, 5000, ExShowScreenMessage.ScreenMessageAlign.TOP_CENTER, true, 1, -1, true));
        }
    }
    ...
    public void spazionSpawn() {
        for (Player p : getPlayers()) {
            ...
            p.sendPacket(new ExShowScreenMessage(NpcString.RESCUED_CHANGES_STATE_ONLY_AFTER_EXPOSURE_TO_LIGHT_IT, 5000, ExShowScreenMessage.ScreenMessageAlign.TOP_CENTER, true, 1, -1, true));
        }
